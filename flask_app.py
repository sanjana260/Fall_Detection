...
from fall_detection_integrated import run_fall_detection_live

# _source_path = Path("fall_detection_integrated.py")
...
def watch_falls():
    ...
    while True:
        ...
        for jf in json_files:
            ...
            screenshot_path = f"Outputs/vigilens_live/people/{fall_data.get('person_id')}"
            ...
            ui_evt = {
                ...
                "fall_data": {
                    ...
                    "scene_context":fall_data.get("scene_context"),
...
def detection_thread():
    print("Running detection thread")
    if os.path.isfile(RECOVERIES_FILE):
        os.remove(RECOVERIES_FILE)
    run_fall_detection_live(0)
...
def fall_screenshot(fall_id):
    # logging.debug("Screenshot requested for fall id: ",fall_id)
    with fall_events_lock:
        for evt in fall_events:
            if evt["id"] == fall_id and evt.get("_screenshot_path"):
                # logging.debug(evt.get("_screenshot_path"))
                screenshot_path = evt["_screenshot_path"]
                screenshot_path = glob.glob(f"{screenshot_path}_*.jpg")[0]
                logging.debug(screenshot_path)
...
def serve_output(filename):
    logging.debug(f"Sending file Outputs/{filename}")
